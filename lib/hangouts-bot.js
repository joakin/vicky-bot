// Inspired from https://www.npmjs.com/package/hangouts-bot
...
module.exports = function (username, password, onlineStatus) {
  ...
  connection.on('offline', function () { that.emit('offline') })

  connection.on('connect', function () { that.emit('connect') })
  ...
  connection.on('disconnect', function (e) {
    that.emit('disconnect', connection.connection.reconnect, e)
  })

  connection.on('error', function (e) { that.emit('error', e) })
