...
module.exports = function () {
  ...
  return function (from, msg) {
    ...
    if (match) {
      ...
      const results = search(terms)
        .then((res) => {
          if (ppl[from].status === 'searching' && ppl[from].q === terms) {
            ...
            return res.pages.map((p, i) => `${CHARS[i]} )  ${p.title}`).join('\n')
          }
        })
      ...
      if (msg.toLowerCase().match(CHARSREGEXP) && validKeys.indexOf(msg) !== -1) {
        ppl[from].status = null
        return [{ type: 'command', value: `show ${ppl[from].results[msg]}` }]
      } else {
        return [
